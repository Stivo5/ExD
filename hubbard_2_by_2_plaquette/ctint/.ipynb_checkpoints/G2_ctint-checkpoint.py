...
for the Hubbard dimer (see ../pomerol_ref/G2_pomerol.py).
...
#   sum eps_a n_{s,a} - h sum_a (n_{up,a} - n_{dn,a}) + t sum_s (c^+_{s0} c_{s1} + h.c.) + U sum_a n_{up,a} n_{dn,a}
h_int = U * sum(n('up', a) * n('dn', a) for a in atoms)

# Quadratic part of H, one matrix per spin block
sigma = {'up': +1.0, 'dn': -1.0}
h0 = {s: np.array([[eps[0] - sigma[s] * h_field, t],
                   [t, eps[1] - sigma[s] * h_field]]) for s in spin_names}
