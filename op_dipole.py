...
def get_char(i_mb_st,fops,orb_names,spin_names,ad,eigensys,Dij,cmplx):
    ...
    for i_state,state in enumerate(eigensys[i_mb_st][0][1]):
        ...
        op_on_vacuum += coeff1*op_vac # When acting on vacuum, creates a transformed state of the state i_mb_st
    ...
    # For constrained occupancy we need to do this without the vacuum state
        ...
        # Transformed state in computational basis
        state_r_subs=[]
        for sub in range(ad.n_subspaces):
            state_r_subs.append(np.zeros(len(ad.fock_states[sub]),dtype='complex128'))

        for iop,op in enumerate(op_on_vacuum):  # op is (generically) a pair ([(True, ['down', 0])], -1.0)
            fock=0
            # Create folk state in computational basis created by op
            for o in op[0]:
                c_op=o[1]
        ...
        # Convert the transformed state into eigenvalue basis by taking inner product with eigenvectors
        for sub in range(ad.n_subspaces):
...
def get_char_table(pg_symbol):
    ...
    if pg_symbol=='m-3m' or pg_symbol=='Oh':
        ...
        # Index of group elements in each conjugacy classes. 24 elements in total in -43m
...
def get_irrep_projection(pg_symbol,orb_names,spin_names,ad,eigensys,n_print,repsfile=[],dij=[],spin=False,cmplx=False,out_label='',verbose=True):
    ...
    if spin:
        ...
        h=np.sum(np.array(char_table['sym_mult'])) # Total number of group elements
    ...
    for i_mb_st in range(n_print[0],min(n_print[1],len(eigensys))):
        ...
        # Loop through irreps
        # Create projection onto each irrep
        proj_irreps=[]
        for irrep in range(len(irrep_labels)):
            proj=0
            # Loop through group elments
            for sym_el in range(h):
                # Get the single-particle rep for this symmetry element
                rep=dij[sym_el]

                # Find the index of conjugacy class for this element
                ...
                # characters[irrep,0] is the dimension of the irrep
                ...
                proj+= proj_R   # proj = projection onto irrep acting on state i_mb_st
            ...
            proj_irreps.append(proj)    # a list of projections of state i_mb_st onto each irrep
...
def weak_proj_tij(pg_symbol,orb_names,spin_names,dij,cfs=[],spin=False):
    ...
    char_table=get_char_table(pg_symbol)
    characters=char_table['characters']
    h=np.sum(np.array(char_table['sym_mult'])) # Size of the group
    n_ops=len(char_table['sym_names']) # Number of conjugacy classes = Number of irreps
    ...
    for ispin in range(n_spin):
        ...
        for icf,cf in enumerate(cfs[ispin]):
            ...
            # Cut of a n_orb by n_orb submatrix from the projection matrix 
...
#*************************************************************************************
# Electron-hole correlation function in real frequency
def get_el_h_L_f(T,ad,spin_names,orb_names,eigensys,n_states,w_min,w_max,n_w,eta=0.01):
...
def get_el_h_L_w(T,ad,spin_names,orb_names,eigensys,n_states,n_iw, eta = 0.01):
    ...
    for iw in iw_mesh:
        L=0
        for dm in den_mats:
            L+=(1j * (np.exp(-beta*dm[0])-np.exp(-beta*dm[1]))*dm[3] +eta) / (1j * iw.value +dm[0]-dm[1]+eta)
            #L+=1j * dm[2] / (omega+dm[0]+1j*eta)
