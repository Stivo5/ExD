...
def sort_states(spin_names,orb_names,ad,n_print,out_label,skip_diff_N=True,prt_mrchar=False,prt_state=True,prt_dm=True,target_mu=5,prt_ad=False,ml_order=[],verbose=True):
    ...
    for sub in range(0,ad.n_subspaces):
        ...
        skip_sub=False
        # get fock state in computation basis by using binary representation
        subspace_fock_state=[]
        for fs in ad.fock_states[sub]:
            state =int(bin(int(fs))[2:]) # Converting to binary representation
            ...
            fmt='{0:0'+str(state_leng)+'d}' # to fill in the leading 0's in computational basis
            state_bin="|"+fmt.format(state)+">"
            subspace_fock_state.append(state_bin)
            print('sub is ', sub, 'fs is ', state_bin)
